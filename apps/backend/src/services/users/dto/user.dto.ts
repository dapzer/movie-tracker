import { SignUpMethodEnum, UserMediaRatingsAccessLevelEnum, UserRoleEnum } from "@movie-tracker/types"
import { createZodDto } from "nestjs-zod"
import { z } from "zod"
import { zDateTimeString } from "@/shared/dto/zod.utils"

export const userSchema = z.object({
  id: z.uuid().meta({ format: "uuid", example: "c1a9b6e2-3f4d-4a7c-9d2e-123456789abc" }),
  name: z.string().meta({ example: "John Doe" }),
  email: z.email().nullish().meta({ example: "user@example.com" }),
  image: z.url().nullish().meta({ example: "https://cdn.example.com/avatar.jpg" }),
